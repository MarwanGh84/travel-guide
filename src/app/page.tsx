...
export default async function Home() {
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-y-auto bg-background p-4 sm:p-8">
      ...
      <header className="mb-6 flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0">
           ...
           <h1 className="mt-2 text-3xl sm:text-5xl font-black tracking-tighter uppercase leading-tight sm:leading-none truncate">{trip.destination ?? trip.name}</h1>
           <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-xs font-bold text-muted">
              <span className="flex items-center gap-2 whitespace-nowrap"><Calendar size={14} /> {trip.startDate} — {trip.endDate}</span>
              <div className="hidden h-4 w-px bg-border sm:block" />
              <span className="flex items-center gap-2 whitespace-nowrap"><MapPin size={14} /> {trip.destinationCountry}</span>
              <div className="hidden h-4 w-px bg-border sm:block" />
              <span className="flex items-center gap-2 whitespace-nowrap"><Sparkles size={14} /> {selectedPlaces.length} SAVED</span>
              <div className="hidden h-4 w-px bg-border sm:block" />
              <span className="flex items-center gap-2 whitespace-nowrap"><Plane size={14} /> {dbTrip?.bookings.length ?? 0} BOOKINGS</span>
              <div className="hidden h-4 w-px bg-border sm:block" />
              <span className="text-foreground flex items-center gap-2 whitespace-nowrap"><CreditCard size={14} /> {formatCurrency(trip.budget)} TOTAL</span>
           </div>
        </div>
        
        <Link href={nextStep.href} className="w-full sm:w-auto">
          <button className="flex h-12 w-full items-center justify-center gap-3 rounded-lg bg-black px-6 text-xs font-bold uppercase tracking-[0.1em] text-white shadow-xl hover:translate-y-[-2px] transition-all sm:w-auto">
      ...
      <div className="mb-10 flex items-center gap-4 overflow-x-auto rounded-xl border border-border bg-surface p-4 shadow-sm scrollbar-hide no-scrollbar">
        {steps.map((step, i) => (
          <div key={step.label} className="flex shrink-0 items-center last:flex-none">
            ...
            {i < steps.length - 1 && (
              <div className={`mx-4 h-px w-8 sm:w-12 lg:w-16 ${step.status === "done" ? "bg-black" : "bg-border"}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <main className="flex-1 min-h-0">
        <div className="flex flex-col gap-6 lg:grid lg:grid-cols-12 lg:grid-rows-6">
          
          {/* Main Visual - The Destination */}
          <section className="relative min-h-[300px] overflow-hidden rounded-2xl border border-border shadow-sm lg:col-span-7 lg:row-span-4">
             ...
             <div className="absolute bottom-6 left-6 sm:bottom-8 sm:left-8 text-white">
                ...
                <div className="mt-2 flex items-center gap-4">
                   <span className="text-4xl sm:text-5xl font-black tracking-tighter">{weather.temperatureRange}</span>
          ...
          <section className="flex flex-col overflow-hidden rounded-2xl border border-border bg-surface lg:col-span-5 lg:row-span-3">
             ...
             <div className="max-h-[300px] flex-1 overflow-y-auto divide-y divide-border/40 lg:max-h-none">
          ...
          <section className="flex flex-col overflow-hidden rounded-2xl border border-border bg-surface-2 lg:col-span-5 lg:row-span-1">
             ...
             <div className="flex-1 p-3 flex gap-2 overflow-x-auto no-scrollbar scrollbar-hide">
          ...
          {/* Budget Snapshot */}
          <section className="flex flex-col justify-between rounded-2xl border border-border bg-background p-6 shadow-sm lg:col-span-3 lg:row-span-2">
             <div>
          ...
          <section className="flex flex-col overflow-hidden rounded-2xl border border-border bg-surface lg:col-span-9 lg:row-span-2">
             ...
             <div className="flex min-h-[140px] flex-1 gap-px bg-border/40">
      ...
      <footer className="mt-8 flex flex-wrap items-center justify-center gap-x-12 gap-y-6 border-t border-border/50 pt-6 pb-4 sm:pb-0">
