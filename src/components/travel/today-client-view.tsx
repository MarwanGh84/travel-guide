...
export function TodayClientView({ trip, days, places, weather }: TodayClientViewProps) {
  ...
  useEffect(() => {
    ...
    // Update every minute to handle midnight crossing or time progression
  ...
  const stateInfo = useMemo(() => {
    ...
    const dStart = new Date(new Date(trip.startDate).toISOString().slice(0, 10) + "T00:00:00Z");
    const dEnd = new Date(new Date(trip.endDate).toISOString().slice(0, 10) + "T00:00:00Z");
    ...
    return { state, todayStr, activeDay, nextBooking, spent, budgetRemaining, nextPlace };
  }, [now, trip, days]);
  ...
  const { state, activeDay, nextBooking, spent, budgetRemaining, nextPlace } = stateInfo;

  if (state === "before") {
    const dToday = new Date(`${stateInfo.todayStr}T00:00:00Z`);
    const dStart = new Date(new Date(trip.startDate).toISOString().slice(0, 10) + "T00:00:00Z");
           ...
           <div className="mx-auto max-w-4xl space-y-12">
              <section className="grid gap-8 lg:grid-cols-2">
                 <div className="space-y-6">
                    ...
                    <h1 className="text-3xl sm:text-5xl lg:text-6xl font-black uppercase tracking-tighter leading-[0.95]">
                       {trip.destination || "The Journey"} <br/> <span className="text-muted/40">is approaching</span>
                    </h1>
                    <p className="text-[10px] font-bold text-muted uppercase tracking-[0.3em] leading-relaxed mt-4">
                 ...
                 <div className="rounded-2xl border-2 border-foreground p-8 flex flex-col justify-between bg-background shadow-xl transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                    <div>
                       <p className="text-[10px] font-black uppercase tracking-widest text-muted mb-4">Readiness Score</p>
                       <p className="text-5xl font-black tracking-tighter leading-none">{checklist.length > 0 ? Math.round((completedItems / checklist.length) * 100) : 0}%</p>
                    ...
                    <div className="mt-8 space-y-3">
                       {missingCrucial.map(item => (
                         <div key={item.id} className="flex items-center gap-2 text-[10px] font-black text-rose-600 uppercase tracking-widest">
                            <AlertTriangle size={12} className="shrink-0" /> Missing: {item.label}
                         </div>
                       ))}
                    </div>
                 </div>
        ...
        <main className="flex-1 overflow-y-auto p-6 sm:p-12 lg:p-24 scrollbar-hide">
           <div className="mx-auto max-w-3xl space-y-16">
              <section className="text-center space-y-6">
                 ...
                 <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black uppercase tracking-tighter leading-none">Welcome Back</h1>
              ...
              <div className="grid gap-6 sm:grid-cols-2">
                 <Link href="/memories" className="group p-8 rounded-2xl border-2 border-border bg-background hover:border-black transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-[0.98]">
                    ...
                    <p className="mt-2 text-[10px] font-bold text-muted uppercase tracking-widest">Archive photos and highlights in the cloud</p>
                 </Link>
                 <Link href="/budget" className="group p-8 rounded-2xl border-2 border-border bg-background hover:border-black transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-[0.98]">
                    ...
                    <p className="mt-2 text-[10px] font-bold text-muted uppercase tracking-widest">Total spent: {formatCurrency(spent, trip.currency)}</p>
                 </Link>
              </div>
        ...
        <section className="flex-1 overflow-y-auto p-8 lg:p-12 xl:p-16 bg-background border-r border-border scrollbar-hide">
           <div className="max-w-3xl mx-auto space-y-12">
              <header>
                 ...
                 <h1 className="mt-4 text-4xl lg:text-5xl font-black tracking-tighter uppercase text-foreground leading-[0.95]">{activeDay.theme}</h1>
              </header>
              ...
              {activeDay.notes && (
                <section className="rounded-xl border border-border bg-surface-2 p-6 italic text-muted-2 text-sm leading-relaxed shadow-inner">
                   {activeDay.notes}
                </section>
              )}

              {nextPlace && (
                <section className="rounded-2xl border-2 border-foreground p-8 bg-background shadow-xl flex flex-col sm:flex-row items-center justify-between gap-6 transition-all duration-300 hover:shadow-2xl">
                   <div className="flex items-center gap-6">
                      <div className="grid size-14 place-items-center rounded-xl bg-foreground text-background shadow-lg">
                         <MapPin size={28} />
                      </div>
                      <div>
                         <p className="text-[10px] font-black uppercase tracking-widest text-muted">Next Objective</p>
                         <h3 className="text-2xl font-black uppercase tracking-tight leading-none mt-1 text-foreground">{nextPlace.title}</h3>
                         {nextPlace.place?.location && <p className="text-[9px] font-bold text-muted uppercase tracking-widest mt-2">{nextPlace.place.location}</p>}
                      </div>
                   </div>
                   {nextPlace.place?.coordinates && (
                      ...
                      >
                        <button className="flex h-12 w-full sm:w-auto items-center justify-center gap-2 rounded-lg bg-foreground px-8 text-[10px] font-black uppercase tracking-widest text-background hover:bg-zinc-800 dark:hover:bg-zinc-200 transition-all active:scale-95 shadow-lg">
              ...
              {nextBooking ? (
                <article className="rounded-xl border border-border bg-background p-5 shadow-sm">
                   <div className="flex items-center justify-between mb-2">
                      <h4 className="text-xs font-black uppercase tracking-tight text-foreground">{nextBooking.title}</h4>
                      {nextBooking.type === "Flight" && (
                         <div className="flex items-center gap-1.5 rounded-full bg-foreground text-background px-2 py-0.5 text-[8px] font-black uppercase tracking-widest">
                ...
                <div className="rounded-xl border border-dashed border-border p-6 text-center opacity-40 bg-background/50">
                   <p className="text-[10px] font-bold uppercase tracking-widest text-muted">No segments identified</p>
                </div>
              )}
           </section>
...
function PreparingCard({ title, status, icon: Icon, href, done }: { title: string, status: string, icon: LucideIcon, href: string, done: boolean }) {
  return (
    <Link href={href} className="group relative flex flex-col justify-between rounded-xl border-2 border-border bg-background p-6 hover:border-foreground transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-[0.97]">
       <div className="flex items-center justify-between mb-4">
          <div className={cn("grid size-12 place-items-center rounded-lg border transition-colors shadow-sm", done ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-500 border-emerald-500/20" : "bg-surface-2 text-muted border-border")}>
             <Icon size={20} />
          </div>
       ...
       <div>
          ...
          <p className="mt-1 text-sm font-black uppercase tracking-tight text-foreground">{status}</p>
       </div>
    </Link>
  );
}
...
function AgendaNode({ marker, title, content }: { marker: string, title: string, content?: string }) {
  return (
    <div className="relative pl-24 sm:pl-32 before:absolute before:left-[11px] before:top-8 before:h-full before:w-px before:bg-border last:before:hidden">
       <span className="absolute left-0 top-1 text-[10px] font-black font-mono text-muted tracking-widest">{marker}</span>
       ...
       <div className="space-y-3">
          ...
          <p className="text-xl sm:text-2xl font-bold leading-relaxed text-foreground tracking-tight max-w-[50ch]">
